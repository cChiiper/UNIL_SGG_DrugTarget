############################################################################################################
### Script to prepare and subset data to apply Mendelian randomization (MR) ################################
...
# Add arguments
parser$add_argument('-e_f', '--expo_df', help = 'Exposure data frame', required = TRUE)
parser$add_argument('-o_f', '--outcome_df', help = 'Outcome data frame', required = TRUE)
parser$add_argument('-s_f', '--snps', help = 'SNPs file', required = TRUE)
parser$add_argument('-o', '--output_df', help = 'Output file', required = TRUE)

# Parse arguments
args <- parser$parse_args()

arg_expo_df <- args$expo_df
arg_outcome_df <- args$outcome_df
arg_snps <- args$snps
arg_output_df <- args$output_df


### Prepare MR data
expo_df <- read.table(arg_expo_df, header = T)
print("Exposure df is in memory.")
outcome_df <- read.table(arg_outcome_df, header = T)
print("Outcome df is in memory..")

snps <- read.table(arg_snps)
names(snps) <- c('SNP')
...
write.table(df, arg_output_df, row.names = F, quote = F)